...
sys.path.insert(0, str(Path(__file__).parent.parent))   #Before searching anywhere else(for importing files), also search the project/ folder for modules.
...
for key, default in [
    ...
    if key not in st.session_state:     # created only for fisrt time
...
if (
    ...
    and "code" in st.query_params         #URL contains code
):
    from frontend.services import supabase_client
    result = supabase_client.exchange_code_for_session(st.query_params["code"])

    #Always clear the ?code= param so a refresh(rerun of streamlit) doesn't try to re-do.
