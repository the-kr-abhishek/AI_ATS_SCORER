...
_client=None               # glabal keyword

def _get_client()->Groq:   # Singleton pattern in LLMs
    global _client
    if _client is None:
...
def _call_groq(client:Groq, system_prompt:str, user_prompt:str)->str:
    ...
    return response.choices[0].message.content.strip()    # returns plain text

def _try_parse_json(text: str) -> dict | None:            # converts groq's plain text to dictionary
